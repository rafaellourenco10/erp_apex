...
**✅ Handler de edição — criado e testado em 2026-08-26.** Não existia nenhum resource template `clientes/:id` no módulo `erp.api` — o Postman devolvia `404 Not Found`. O código Flutter (`ClienteService.atualizarCliente`) já chamava esse endpoint desde o commit `68c230a`, mas o passo manual de criá-lo no App Builder nunca tinha sido feito (a mensagem de commit dizia "aplicado, ainda sem teste confirmado" — na prática, não estava). Criado o template `clientes/:id` + handler `POST`, mesmo padrão dos outros (`JSON_VALUE` pros 11 campos, `UPDATE ... WHERE id_cliente = :id`, `:status_code := 400` + `{"error": SQLERRM}` em erro, e uma checagem extra de `SQL%ROWCOUNT = 0` pra erro claro se o id não existir). Testado via Postman: `200 OK`, `{"id_cliente": 21}`.
...
- **`POST /clientes/:id`** (template novo) — edita cliente (mesmo body do create, incluindo os 8 campos de CPF/CNPJ e endereço desde 2026-08-25). Erro `ORA-20006` se `nome` vier vazio, `ORA-20007` se o `:id` não existir. **✅ Criado e testado em 2026-08-26** — ver detalhe acima e em `docs/MEMORIA.md` (o template não existia de fato até essa data, apesar de o código Flutter já assumir sua existência desde 2026-08-24).
